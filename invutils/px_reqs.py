...
def gecko_current(id_gecko:str, vs_currencies:str = 'usd'):
  ...
  Returns:
    json: {'bitcoin': {'currency': price}, {'ethereum': {'currency': price}}, ...}
  """
  ...
  try:
    res = requests.get(url,params = {'ids': id_gecko, 'vs_currencies': vs_currencies})
...
def gecko_hist(id_gecko:str, vs_currency:str = 'usd', days = 'max'):
  ...
  Returns:
    json: {'prices': [[date(ms), price], [date(ms), price], ...]}
  """
  assert type(id_gecko) is str, 'id_gecko should be a str'
...
def llama_hist(id_llama:str, timestamp = int(time.mktime(datetime.now().timetuple()))):
  ...
  Returns:
    json: {defillama_id: {..., 'symbol': symbol, 'price': price}} - Has some other data
  """
